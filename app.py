...
if run_btn:
    ...
    for opt_idx in range(3):
        ...
        # We constrain that the sum of roster variables matching the previous solution must be < N
        for prev_sol_roster in previous_solutions_constraints:
            # prev_sol_roster is a list of (pid, d_idx) tuples that were 1
            # Constraint: Sum(vars that were 1) <= Count - 1
                            ...
                            if eid in past_event_ids:
                                if eid in past_day_stats:
                                    ...
                                    for pick in stats['picks']:
                                        ...
                                        # Get actual score
                                        actual_pts = get_player_score_for_date(pid, event_dates.get(eid, '?'))
